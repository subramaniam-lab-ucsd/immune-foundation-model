...
dist.init_process_group(backend="nccl")  # Use "nccl" for GPUs, "gloo" for CPUs

# Get the local rank (set automatically by torchrun)
...
for epoch in range(1, epochs + 1):  # epochs is the maximum number of epochs
    ...
    # Use DistributedSampler for training and validation
    train_sampler = DistributedSampler(train_data_pt, shuffle=True)
    valid_sampler = DistributedSampler(valid_data_pt, shuffle=False)
    ...
    # Train for one epoch
    if config.do_train:
        train_sampler.set_epoch(epoch)  # Ensure proper shuffling for DistributedSampler
